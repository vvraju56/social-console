...
@router.get("/media")
async def instagram_media(ig_user_id: Optional[str] = None, token: Optional[str] = None, user_id: Optional[str] = None, date_range: Optional[str] = "30d"):
    access_token = token
    ig_uid = ig_user_id
    if (not access_token or not ig_uid) and user_id:
        user = supabase.table("users").select("instagram_token, ig_user_id").eq("id", user_id).single().execute().data
    ...
    limit = 25 if date_range in ["30d", "90d", "all"] else 10
    r = await client.get(f"{IG_GRAPH}/{ig_uid}/media", params={"fields": fields, "access_token": access_token, "limit": limit})
    r.raise_for_status()
    return r.json()
