...
async def youtube_analytics(token: Optional[str] = None, user_id: Optional[str] = None, channel_id: Optional[str] = None, date_range: Optional[str] = "30d"):
    # OAuth path
    access_token = token
    if not access_token and user_id and supabase is not None:
        user = supabase.table("users").select("youtube_token").eq("id", user_id).single().execute().data
    ...
    if access_token:
        headers = {"Authorization": f"Bearer {access_token}"}
        # Adjust max results based on date range
        max_results = 50 if date_range in ["30d", "90d", "all"] else 10
        search_params = {"part": "id", "forMine": True, "type": "video", "maxResults": max_results, "order": "date"}
    ...
    # API key public fallback
    if channel_id and settings.YOUTUBE_API_KEY:
        # latest videos from channel using search
        max_results = 50 if date_range in ["30d", "90d", "all"] else 10
        search_params = {
            "part": "id",
